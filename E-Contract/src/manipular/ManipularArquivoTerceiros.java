...
public class ManipularArquivoTerceiros {
	...
	public CadastroContrato filtrar(File file) {
		...
		try (PDDocument document = PDDocument.load(file)) {
			...
			if (!document.isEncrypted()) {
    			...
    			//valor
    			String valor =  tratamentoDados.tratar(" foi estabelecido entre as partes", "saca").replaceAll("[^0-9,]+", "").replaceFirst(",", "");
    			double valor_double = new Double(valor.replace(",", "."));
    			...
    			for(int i = 0; i < lines.length; i++) {
    			 ...
    			 String conta_corrente =   tratamentoDados.tratar("Conta Corrente: ", ",").replace(",", "").trim(); 
    			...
    			if(vendedor_cadastrado) {
    				...
    				if(vendedor_contrato.getIe().equals(inscricao_contrato)) {
    					...
    					//informa ao usuario que o cliente possui o contrato, mas a inscricao estadual nao e a mesma, 
