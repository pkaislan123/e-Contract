...
public class GerenciarBancoFuncionarios {
				...
				if (id_funcionario > 0) {
					...
					if (reverter_operacao) {
						...
						JOptionPane.showMessageDialog(null,
								"Erro ao inserir o funcionario no banco de dados, revertendo operacao");
						if (deletar_contatos) {
					...
					JOptionPane.showMessageDialog(null,
							"Erro ao inserir o funcionario no banco de dados\nBanco Normalizado! ");
					retorno = false;
				}
	...
	public boolean remover_funcionario(int id_funcionario) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao excluir o funcionario do banco de" + "dados " + f.getMessage());
			return false;
		}
	}
	...
	public boolean deleteContato(int id_contato, int id_funcionario) {
		...
		try {
			...
			pstm.execute();
			ConexaoBanco.fechaConexao(conn, pstm);
			// JOptionPane.showMessageDialog(null, "Relação funcionario_contato excluido,
			// banco normalizado ");
			...
			JOptionPane.showMessageDialog(null,
					"Erro a relação funcionario_contato do banco de" + "dados " + f.getMessage());
			return false;
	...
	public boolean deleteConta(int id_conta, int id_funcionario) {
		...
		System.out.println("Inicio de exclusão de conta bancaria: ID da Conta: " + id_conta + " ID do funcionario: "
				+ id_funcionario);
		Connection conn = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Relação funcionario_conta excluido,
			// banco normalizado ");
			...
			JOptionPane.showMessageDialog(null,
					"Erro a relação funcionario_conta do banco de" + "dados " + f.getMessage());
			return false;
	...
	public CadastroFuncionario getfuncionario(int id_funcionario) {
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Erro ao listar funcionario(gbc) id: " +
			// id_funcionario );
			return null;
		}
	...
	public RegistroAdicionarContaBancaria adicionarContaBancaria(ArrayList<ContaBancaria> contas, int id_funcionario) {
		...
		for (int i = 0; i < ids_contas.size(); i++) {
			System.out.println("Inicio de Adição de Relação Clinte-Conta Bancaria, ID funcionario: " + id_funcionario
			...
			String sql_cadastro_funcionario_conta = "insert into funcionario_conta (id_funcionario , id_conta ) values ('"
					...
					+ id_funcionario + "','" + ids_contas.get(i) + "')";
			...
			try {
				...
				grava.execute();
				// JOptionPane.showMessageDialog(null, "funcionario Conta bancaria cadastrado
				// com sucesso");
				ConexaoBanco.fechaConexao(conn, grava);
				resposta = true;
				registro.setIds_contas(ids_contas);
	...
	public RegistroAdicionarContato adicionarContato(ArrayList<Contato> contatos, int id_funcionario) {
		...
		for (int i = 0; i < ids_contatos.size(); i++) {
			String sql_cadastro_funcionario_contato = "insert into funcionario_contato (id_funcionario , id_contato ) values ('"
					...
					+ id_funcionario + "','" + ids_contatos.get(i) + "')";
			...
			try {
				...
				grava.execute();
				// JOptionPane.showMessageDialog(null, "funcionario Contato cadastrado com
				// sucesso");
				ConexaoBanco.fechaConexao(conn, grava);
				resposta = true;
				registro.setIds_contatos(ids_contatos);
	...
	public String string_pessoa_fisica(CadastroFuncionario funcionario) {
		return "insert into funcionario (nome, sobrenome, cpf, rg, estado_pessoa, nacionalidade, naturalidade, nascimento,"
				...
				+ " titulo_eleitor, titulo_secao, titulo_zona," + " certida_militar, certiao_serie, certidao_categoria,"
				...
				+ funcionario.getNome_pai() + "','" + funcionario.getApelido() + "','" + funcionario.getRua() + "','"
				+ funcionario.getBairro() + "','" + funcionario.getCep() + "','" + funcionario.getCidade() + "','"
				+ funcionario.getNumero() + "','" + funcionario.getEstado_endereco() + "','"
				+ funcionario.getTitulo_eleitor() + "','" + funcionario.getTitulo_secao() + "','"
				+ funcionario.getTitulo_zona() + "','" + funcionario.getCertida_militar() + "','"
				+ funcionario.getCertiao_serie() + "','" + funcionario.getCertidao_categoria() + "','"
				+ funcionario.getPis() + "','" + funcionario.getNis() + "','" + funcionario.getCtps_serie() + "','"
				+ funcionario.getCtps_estado() + "','" + funcionario.getCnh_categoria() + "','"
				+ funcionario.getCnh_validade() + "','" + funcionario.getCnh_num_registro() + "','"
				+ funcionario.getGrau_escolaridade() + "','" + funcionario.getCursos() + "','"
	...
	public ArrayList<FuncionarioContaAssociada> buscarContasAssociadas(int id_colaborador) {
		String selectfuncionarios;

		// busca padrao
		selectfuncionarios = "select * from \r\n"
				+ "funcionario_conta_associada fca\r\n"
				+ "left join cliente cc on cc.id_cliente = fca.id_cliente\r\n"
				+ "where fca.id_funcionario = ?";

		Connection conn = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		ArrayList<FuncionarioContaAssociada> lista_clientes = new ArrayList<>();
		try {
			conn = ConexaoBanco.getConexao();
			pstm = conn.prepareStatement(selectfuncionarios);
			pstm.setInt(1, id_colaborador);
			...
			while (rs.next()) {
				...
				fca.setId_funcionario(id_colaborador);
				fca.setCliente(cliente);
				
				lista_clientes.add(fca);
			...
			return lista_clientes;
			 ...
			 JOptionPane.showMessageDialog(null, "Erro ao listar Clientes Associados!\nErro: " + e.getMessage() + "\nCausa: " +e.getCause() );
			 return null;
		}

	}
	
	public boolean deleteContaAssociada(int id_func, int id_cliente) {

		
		Connection conn = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		String sql_delete_relacao = "DELETE FROM funcionario_conta_associada WHERE id_funcionario = ? and id_cliente = ?";
		conn = null;
		try {
			conn = ConexaoBanco.getConexao();
			pstm = conn.prepareStatement(sql_delete_relacao);
			...
			ConexaoBanco.fechaConexao(conn, pstm);
				...
				JOptionPane.showMessageDialog(null,
						"Erro ao excluir a associacão do banco de dados " + g.getMessage());
				return false;
	...
	public boolean inserirFuncionarioContaAssociada(int id_func, int id_cliente) {
			Connection conn = null;
			String atualizar = null;
			PreparedStatement pstm;
			...
			try {
				...
				atualizar = "insert into funcionario_conta_associada (id_funcionario, id_cliente) values (?, ?)";
				...
				JOptionPane.showMessageDialog(null, "Erro ao inserir Relação Funcionário Cliente no banco de dados\nErro: " + e.getMessage() + "\nCausa: " + e.getCause());
				return false;
			}
