...
public class GerenciarBancoClientes {
		...
		if (cliente != null) {
			...
			try {
				...
				if (id_cliente > 0) {
					...
					if (contas_bancarias.size() > 0 || contatos.size() > 0 || veiculos.size() > 0) {
						...
						if (prosseguir) {
							if (cliente.getTransportador() == 1) {
								if (veiculos.size() > 0) {
									...
									if (adicionar_veiculos.isResposta() == true
											&& adicionar_veiculos.ids_veiculos.size() > 0) {
						...
						if (reverter_operacao) {
							...
							JOptionPane.showMessageDialog(null,
									"Erro ao inserir o Cliente no banco de dados, revertendo operacao");
							if (deletar_contatos) {
					...
					JOptionPane.showMessageDialog(null,
							"Erro ao inserir o Cliente no banco de dados\nBanco Normalizado! ");
					retorno = false;
				}
	...
	public boolean remover_cliente(int id_cliente) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Erro ao excluir o cliente do banco de" + "dados " + f.getMessage());
			return false;
		}
	}
	...
	public int inserir_cliente(CadastroCliente cliente) {
		...
		if (cliente.getTipo_pessoa() == 0) { // 0 pessoa fisica
			if (cliente.getTransportador() == 1) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao inserir cliente no banco de dados\nErro: " + e.getMessage() + "\nCausa:" + e.getCause());
			return -1;
		}
	...
	public boolean remover_conta(int id_conta) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao excluir o conta bancaria do banco de" + "dados " + f.getMessage());
			return false;
		}
	}
	...
	public boolean remover_contato(int id_contato) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null, "Erro ao excluir o contato do banco de" + "dados " + f.getMessage());
			return false;
	...
	public boolean deleteContato(int id_contato, int id_cliente) {
		...
		try {
			...
			pstm.execute();
			ConexaoBanco.fechaConexao(conn, pstm);
			// JOptionPane.showMessageDialog(null, "Relação cliente_contato excluido, banco
			// normalizado ");
			...
			try {
				...
				JOptionPane.showMessageDialog(null,
						"Erro ao excluir o contato do banco de" + "dados " + g.getMessage());
				return false;
			...
			JOptionPane.showMessageDialog(null,
					"Erro a relação cliente_contato do banco de" + "dados " + f.getMessage());
			return false;
	...
	public boolean deleteVeiculo(int id_veiculo, int id_cliente) {
		...
		try {
			...
			try {
				...
				JOptionPane.showMessageDialog(null,
						"Erro ao excluir o veiculo do banco de" + "dados " + g.getMessage());
				return false;
			...
			JOptionPane.showMessageDialog(null,
					"Erro a relação transportador_veiculos do banco de" + "dados " + f.getMessage());
			return false;
	...
	public boolean deleteConta(int id_conta, int id_cliente) {
		...
		System.out.println(
				"Inicio de exclusão de conta bancaria: ID da Conta: " + id_conta + " ID do Cliente: " + id_cliente);
		Connection conn = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Relação cliente_conta excluido, banco
			// normalizado ");
			...
			try {
				...
				pstm.execute();
				ConexaoBanco.fechaConexao(conn, pstm);
				// JOptionPane.showMessageDialog(null, "Conta bancaria excluida, banco
				// normalizado ");
				...
				JOptionPane.showMessageDialog(null,
						"Erro ao excluir o conta bancaria do banco de" + "dados " + g.getMessage());
				return false;

			}

		} catch (Exception f) {
			JOptionPane.showMessageDialog(null, "Erro a relação cliente_conta do banco de" + "dados " + f.getMessage());
			return false;
	...
	public ArrayList<CadastroCliente> getClientes(int tipoBusca, int ordem, String campo_busca) {
		...
		if (tipoBusca == 1) {
			...
			// selectClientes = "select * from cliente where id_cliente in (?) or ie in (?)
			// or apelido in (?) or cpf in (?) or cnpj in (?) or nome in (?)";
	...
	public CadastroCliente getCliente(int id_cliente) {
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Erro ao listar cliente(gbc) id: " +
			// id_cliente );
			return null;
		}
	...
	public CadastroCliente getCliente(String nome) {
		String selectClientes = "select * from cliente where nome_empresarial like '%" + nome
				+ "%' or nome_fantasia like '%" + nome + "%' and cliente.armazem = 1";
	...
	public RegistroAdicionarContaBancaria adicionarContaBancaria(ArrayList<ContaBancaria> contas, int id_cliente) {
		...
		for (ContaBancaria conta : contas) {
			String sql_cadastro_conta = "insert into conta_bancaria (cpf, nome, banco, codigo, agencia, conta) values ('"
					...
					+ conta.getCpf_titular() + "','" + conta.getNome() + "','" + conta.getBanco() + "','"
					+ conta.getCodigo() + "','" + conta.getAgencia() + "','" + conta.getConta() + "')";

			try {
				conn = ConexaoBanco.getConexao();

				// PreparedStatement grava = (PreparedStatement)
				// conn.prepareStatement(sql_cdastro_contato);
				// grava.execute();
		...
		for (int i = 0; i < ids_contas.size(); i++) {
			System.out.println("Inicio de Adição de Relação Clinte-Conta Bancaria, ID Cliente: " + id_cliente
					+ " ID Conta: " + ids_contas.get(i));

			String sql_cadastro_cliente_conta = "insert into cliente_conta (id_cliente , id_conta ) values ('"

					+ id_cliente + "','" + ids_contas.get(i) + "')";
			...
			try {
				...
				grava.execute();
				// JOptionPane.showMessageDialog(null, "Cliente Conta bancaria cadastrado com
				// sucesso");
				ConexaoBanco.fechaConexao(conn, grava);
				resposta = true;
				registro.setIds_contas(ids_contas);
	...
	public RegistroAdicionarVeiculos adicionarVeiculos(ArrayList<CadastroCliente.Veiculo> veiculos, int id_cliente) {
		...
		for (CadastroCliente.Veiculo veiculo : veiculos) {
			String sql_cadastro_veiculo = "insert into veiculo (rntrc, placa, eixos, tipo, cidade, estado) values ('"
					...
					+ veiculo.getRegistro_trator() + "','" + veiculo.getPlaca_trator() + "','"
					+ veiculo.getEixos_trator() + "','" + veiculo.getTipo_trator() + "','" + veiculo.getCidade_trator()
		...
		for (int i = 0; i < ids_veiculos.size(); i++) {
			System.out.println("Inicio de Adição de Relação transportador_veiculos , ID Cliente: " + id_cliente
					+ " ID veiculo: " + ids_veiculos.get(i));

			String sql_cadastro_cliente_conta = "insert into transportador_veiculos (id_cliente , id_veiculo ) values ('"

					+ id_cliente + "','" + ids_veiculos.get(i) + "')";
	...
	public RegistroAdicionarContato adicionarContato(ArrayList<Contato> contatos, int id_cliente) {
		...
		for (Contato contato : contatos) {
			...
			String sql_cdastro_contato = "insert into contato (nome_contato, cargo_contato, celular_contato, fixo_contato, e_mail_contato,descricao_contato, observacao_contato) values ('"
					...
					+ contato.getNome() + "','" + contato.getCargo() + "','" + contato.getCelular() + "','"
					+ contato.getFixo() + "','" + contato.getE_mail() + "','" + contato.getDescricao() + "','"
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao inserir Contato no banco de" + "dados ");
			}
		} // fim do for
		...
		for (int i = 0; i < ids_contatos.size(); i++) {
			...
			try {
				...
				grava.execute();
				// JOptionPane.showMessageDialog(null, "Cliente Contato cadastrado com
				// sucesso");
				ConexaoBanco.fechaConexao(conn, grava);
				resposta = true;
				registro.setIds_contatos(ids_contatos);
	...
	public String string_pessoa_juridica(CadastroCliente cliente) {
		return "insert into cliente (tipo_cliente, apelido, razao_social, nome_fantasia, cnpj, descricao, at_primaria, at_secundaria, ie, status_empresa, status_ie, rua, bairro, cep, cidade, numero, uf, tipo_identificacao, identificacao, cpf_responsavel, senha, armazem, transportador) values ('"
				...
				+ cliente.getTipo_pessoa() + "','" + cliente.getApelido() + "','" + cliente.getRazao_social() + "','"
				+ cliente.getNome_fantaia() + "','" + cliente.getCnpj() + "','" + cliente.getDescricao() + "','"
				+ cliente.getAt_primaria() + "','" + cliente.getAt_secundaria() + "','" + cliente.getIe() + "','"
				+ cliente.getStatus() + "','" + cliente.getStatus_ie() + "','" + cliente.getRua() + "','"
				+ cliente.getBairro() + "','" + cliente.getCep() + "','" + cliente.getCidade() + "','"
				+ cliente.getNumero() + "','" + cliente.getUf() + "','" + cliente.getTipo_identificacao() + "','"
				+ cliente.getIdentificacao_sefaz() + "','" + cliente.getCpf_responsavel() + "','" + cliente.getSenha()
				+ "','" + cliente.getArmazem() + "','" + cliente.getTransportador() + "')";
	}
	...
	public String string_pessoa_juridica_transportador(CadastroCliente cliente) {
		return "insert into cliente (tipo_cliente, apelido, razao_social, nome_fantasia, cnpj, descricao, at_primaria, at_secundaria, ie, status_empresa, status_ie, rua, bairro, cep, cidade, numero, uf, tipo_identificacao, identificacao, cpf_responsavel, senha, armazem, transportador) values ('"
				...
				+ cliente.getStatus() + "','" + cliente.getStatus_ie() + "','" + cliente.getRua() + "','"
				+ cliente.getBairro() + "','" + cliente.getCep() + "','" + cliente.getCidade() + "','"
				+ cliente.getNumero() + "','" + cliente.getUf() + "','" + " " + "','" + " " + "','" + " " + "','"
				+ cliente.getSenha() + "','" + cliente.getArmazem() + "','" + cliente.getTransportador() + "')";
	}
	...
	public String string_pessoa_fisica(CadastroCliente cliente) {
		return "insert into cliente (tipo_cliente, apelido, cpf, nome_empresarial, nome, sobrenome, nascimento, rg, ocupacao, porte, atividade, ie, status_empresa, status_ie, rua, bairro, cep, cidade, numero, uf, tipo_identificacao, identificacao, cpf_responsavel, senha, armazem, transportador) values ('"
				...
				+ cliente.getTipo_pessoa() + "','" + cliente.getApelido() + "','" + cliente.getCpf() + "','"
				+ cliente.getNome_empresarial() + "','" + cliente.getNome() + "','" + cliente.getSobrenome() + "','"
				+ cliente.getNascimento() + "','" + cliente.getRg() + "','" + cliente.getOcupacao() + "','"
				+ cliente.getPorte() + "','" + cliente.getAtividade() + "','" + cliente.getIe() + "','"
				+ cliente.getStatus() + "','" + cliente.getStatus_ie() + "','" + cliente.getRua() + "','"
				+ cliente.getBairro() + "','" + cliente.getCep() + "','" + cliente.getCidade() + "','"
				+ cliente.getNumero() + "','" + cliente.getUf() + "','" + cliente.getTipo_identificacao() + "','"
				+ cliente.getIdentificacao_sefaz() + "','" + cliente.getCpf_responsavel() + "','" + cliente.getSenha()
	...
	public ArrayList<CadastroCliente.Veiculo> getVeiculos(int id_cliente) {
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Erro ao listar as veiculos do
			// transportador: " + id_cliente + " erro: " + "causa: " );
			return null;
		}
	...
	public ArrayList<CadastroCliente.Veiculo> getVeiculos() {
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Erro ao listar todos os veiculos erro: "
			// + "causa: " );
			return null;
		}
	...
	public CadastroCliente.Veiculo getVeiculo(int id_veiculo) {
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Erro ao lista o veiculo do id: " +
			// id_veiculo );
			return null;
		}
