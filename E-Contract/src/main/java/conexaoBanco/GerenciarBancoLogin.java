...
public class GerenciarBancoLogin {
	...
	public CadastroLogin buscaLogin(String login) {
		...
		try {
			// String busca = "select * from estoque_filamentos where id in (?) or
			// quantidade in (?) or filamento in (?) or filamentos in (?)";
			// String selectEstoqueFilamentosFiltro =" select estoque_filamentos.id,
			// estoque_filamentos.quantidade, estoque_filamentos.filamento, filamentos.nome
			// from estoque_filamentos join filamentos on estoque_filamentos.filamento =
			// filamentos.id";
	...
	public CadastroLogin getLogin(int id) {
		...
		try {
			...
			// JOptionPane.showMessageDialog(null, "Erro ao buscar id ="+id+" no bando de
			// dados" );
		}
		return Login;
		...
		if (retorno_login_inserido > 0) {
			...
			if (retorno_privilegio_inserido > 0) {
				...
				if (inserir_usuarios_privilegios(retorno_login_inserido, retorno_privilegio_inserido)) {
					...
					if (retorno_preferencias_inserido > 0) {
						...
						if (inserir_usuarios_preferencias(retorno_login_inserido, retorno_preferencias_inserido)) {
							JOptionPane.showMessageDialog(null,
									"relação usaario_preferencias inserido na tabela com sucesso");
							...
							// remover tabela preferencias, usuario preferencias, remover tabela usuario e
							// remover tabela de usuarios e usuarios_privilegios
							...
							boolean remover_Usuarios_privilegios = remover_usuario_privilegios(retorno_login_inserido,
									retorno_privilegio_inserido);
							boolean remover_preferencias = remover_preferencias(retorno_preferencias_inserido);
							boolean remover_Usuarios_preferencias = remover_usuario_preferencias(retorno_login_inserido,
									retorno_preferencias_inserido);

							if (remover_privilegios && remover_usuario && remover_Usuarios_privilegios
									&& remover_preferencias && remover_Usuarios_preferencias) {
								System.out.println(
										"Erro ao cadastrar tabela de usuario_preferencias para o usuario, banco de dados normalizado");

								return 0;
							} else {
								System.out.println(
										"Erro ao remover tabelas de usuario do banco de dados, banco de dados corrompido");
								return -1;
							}
						}
						...
						if (remover_privilegios && remover_usuario && remover_Usuarios_privilegios
								&& remover_preferencias) {
							System.out.println(
									"Erro ao cadastrar tabela de preferencias para o usuario, banco de dados normalizado");

							return 0;
						} else {
							System.out.println(
									"Erro ao remover tabela de usuario do banco de dados, banco de dados corrompido");
							return -1;
						}
					}
					...
					// remover tabela privilegios, remover tabela usuario e remover tabela de
					...
					if (remover_privilegios && remover_usuario && remover_Usuarios_privilegios) {
						System.out.println(
								"Erro ao cadastrar tabela de usuario_privilegios para o usuario, banco de dados normalizado");

						return 0;
					} else {
						System.out.println(
								"Erro ao remover tabela de privilegios e usuario do banco de dados, banco de dados corrompido");
						return -1;
					}
				...
				if (remover_usuario(retorno_login_inserido)) {
					System.out.println(
							"Erro ao cadastrar tabela de privilegios para o usuario, banco de dados normalizado");
					...
					// houve corrupcao no banco de dados, um usuario sem priviegios e direitos esta
					// cadastrado
					return -2;
				}
	...
	private boolean atualizar_usuario(CadastroLogin login) {
		if (login != null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao atualizar usuario no banco de" + "dados ");
				return false;
			}
		} else {
	...
	private boolean atualizarPreferencias(CadastroLogin login) {
		if (login != null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao atualizar preferencias no banco de" + "dados ");
				return false;
			}
		} else {
	...
	private boolean atualizarPrivilegios(CadastroLogin login) {
		if (login != null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao atualizar privilegios no banco de" + "dados ");
				return false;
			}
		} else {
	...
	private boolean remover_usuario(int id_usuario) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao excluir usuario da base de dados\nBanco de dados corrompido!\nConsulte o administrador do sistema"
							+ "dados " + f.getMessage());
			return false;
		}
	...
	private int inserir_privilegios_retorno(CadastroLogin.Privilegios privilegios) {
		...
		if (privilegios != null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao inserir o prvilegios no banco de" + "dados ");
	...
	private boolean remover_privilegios(int id_privilegios) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao excluir privilegio da base de dados\nBanco de dados corrompido!\nConsulte o administrador do sistema"
							+ "dados " + f.getMessage());
			return false;
		}
	...
	private boolean remover_usuario_privilegios(int id_usuario, int id_privilegios) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao exlcuir relação usuarios_privilegios do banco de dados\nBanco de dados corrompido!\nConsulte o administrador do sistema"
							+ "dados " + f.getMessage());
			return false;
		}
	...
	private int inserir_preferencias_retorno(CadastroLogin.Preferencias preferencias) {
		...
		if (preferencias != null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao inserir o preferencias no banco de" + "dados ");
	...
	private boolean remover_preferencias(int id_preferencias) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao excluir Preferencias da base de dados\nBanco de dados corrompido!\nConsulte o administrador do sistema"
							+ "dados " + f.getMessage());
			return false;
		}
	...
	private boolean remover_usuario_preferencias(int id_usuario, int id_preferencias) {
		...
		try {
			...
			JOptionPane.showMessageDialog(null,
					"Erro ao exlcuir relação usuario_preferencias do banco de dados\nBanco de dados corrompido!\nConsulte o administrador do sistema"
							+ "dados " + f.getMessage());
			return false;
		}
	...
	private String sql_privilegios(CadastroLogin.Privilegios privilegios) {
		String sql = "insert into privilegios\r\n" + "(nivel_privilegio, alterar_apis) values ('"
				+ privilegios.getNivel_privilegios() + "','" + privilegios.getPrivilegio_alterar_apis() + "')";
	...
	private String sql_preferencias(CadastroLogin.Preferencias preferencias) {
		String sql = "insert into preferencias\r\n" + "(api_sintegra, api_whatsapp, api_exato) values ('"
				+ preferencias.getApi_sintegra() + "','" + preferencias.getApi_whatsapp() + "','"
	...
	public class Registro {
		...
		// de dados
		// tanto o id_usuario como o id_tabelas para que se houver falha o processo seja
		// desfeito.
	...
	public boolean enviarMensagem(CadastroLogin.Mensagem msg) {
		...
		if (retorno > 0) {
			 ...
			 * return true;
			 * 
			 * }else { System.out.println("Erro ao enviar a mensagem"); return false; }
			 */
			return true;
	...
	private int inserir_mensagem_retorno(CadastroLogin.Mensagem msg) {
		...
		if (msg != null) {
			...
			try {
				...
				JOptionPane.showMessageDialog(null, "Erro ao inserir a mensagem no banco de" + "dados ");
				GerenciadorLog.registrarLogDiario("falha",
						"falha ao inserir mensagem ao banco de dados: " + " causa: ");
				return -1;
			}
		} else {
	 ...
	 * Connection conn = null; try { conn = ConexaoBanco.getConexao(); String sql =
	 * "insert into usuario_mensagens\r\n" + "(id_usuario , id_mensagem ) values ('"
	 * + id_usuario + "','" + id_msg + "')";
	 * 
	 * PreparedStatement grava = (PreparedStatement) conn.prepareStatement(sql);
	 * grava.execute(); ConexaoBanco.fechaConexao(conn, grava);
	 * JOptionPane.showMessageDialog(null,
	 * "Relação usuario_mensagem cadastrado com sucesso");
	 * 
	 * return true;
	 * 
	 * } catch (Exception e) { JOptionPane.showMessageDialog(null,
	 * "Erro ao inserir a relação usuario_mensagens no banco de" +
	 * " dados, usuario: " + id_usuario + " id mensagem: " + id_msg ); return false;
	 * }
	 * 
	 * 
	 * }
	...
	public String sql_msg(CadastroLogin.Mensagem msg) {
		...
		return "insert into mensagem (data_mensagem , hora_mensagem, conteudo, id_remetente, id_destinatario  )  values ('"
				+ msg.getData() + "','" + msg.getHora() + "','" + msg.getConteudo() + "','" + msg.getId_remetente()
