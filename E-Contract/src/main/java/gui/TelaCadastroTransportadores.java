...
public class TelaCadastroTransportadores extends JDialog {
	...
	private JTextFieldPersonalizado entRegistroTrator, entPlacaTrator, entMunicipioTrator, entEstadoTrator,
			entEstadoReboque1, entMunicipioReboque1, entPlacaReboque1, entRegistroReboque1, entRegistroReboque2,
			entPlacaReboque2, entMunicipioReboque2, entEstadoReboque2, entRegistroTransportador;
	...
	public TelaCadastroTransportadores(int flag_tipo_tela, CadastroCliente cliente, TelaTransportadores telaPai,
		...
		table_veiculos.getColumnModel().getColumn(0).setPreferredWidth(20);
		table_veiculos.getColumnModel().getColumn(1).setPreferredWidth(20);
		table_veiculos.getColumnModel().getColumn(2).setPreferredWidth(130);
		table_veiculos.getColumnModel().getColumn(3).setPreferredWidth(30);
		table_veiculos.getColumnModel().getColumn(4).setPreferredWidth(30);
		table_veiculos.getColumnModel().getColumn(5).setPreferredWidth(100);
		...
		entPlacaTrator.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				try {
					
					if(e.getKeyChar() == '\b') {
						String placa_atual = entPlacaTrator.getText();
						
						String placa_atual_back = placa_atual.substring(0, placa_atual.length() );
						entPlacaTrator.setText(placa_atual_back.toUpperCase());

						
					}else {
					e.consume();
					entPlacaTrator.setText((entPlacaTrator.getText() + "" + e.getKeyChar()).toUpperCase());
					}
				} catch (Exception v) {
		...
		btnAdicionarVeiculo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				modelo_veiculos.addRow(new Object[] { id, registro_trator, placa_trator, eixos_trator, tipo_trator,
						cidade_trator, uf_trator });
		...
		btnExcluirVeiculo.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (flag_tipo_tela == 1) {
					...
					((DefaultTableModel) table_veiculos.getModel()).removeRow(indiceDaLinha);
					table_veiculos.repaint();
					table_veiculos.validate();
		...
		entNome.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e) {
				...
				if(e.getKeyChar() == '\b') {
					String nome_atual = entNome.getText();
					
					String nome_back = nome_atual.substring(0, nome_atual.length() );
					entNome.setText(nome_back.toUpperCase());
					entNomeEmpresarial.setText(nome_back.toUpperCase() + " " + entSobrenome.getText().toUpperCase());

					
				}else {
					try {
						e.consume();
						entNome.setText((entNome.getText() + "" + e.getKeyChar()).toUpperCase());
					} catch (Exception v) {

					}
					try {
						entNomeEmpresarial.setText(entNome.getText().toUpperCase() + " " + entSobrenome.getText().toUpperCase());
						entApelido.setText(entNome.getText().toUpperCase());
					} catch (Exception t) {
		...
		entSobrenome.addKeyListener(new KeyAdapter() {
			@Override
			public void keyTyped(KeyEvent e) {
				

				if(e.getKeyChar() == '\b') {
					String sobre_nome_atual = entSobrenome.getText();
					
					String sobre_nome_atual_back = sobre_nome_atual.substring(0, sobre_nome_atual.length() );
					entSobrenome.setText(sobre_nome_atual_back.toUpperCase());
					entNomeEmpresarial.setText(entNome.getText().toUpperCase() + " " + sobre_nome_atual_back.toUpperCase());

					
				}else {
					try {
						e.consume();
						entSobrenome.setText((entSobrenome.getText() + "" + e.getKeyChar()).toUpperCase());
					} catch (Exception v) {
	...
	public void getVeiculos() {
		...
		if (veiculos != null && veiculos.size() > 0) {
			for (CadastroCliente.Veiculo veiculo : veiculos) {
				...
				String registro_trator = veiculo.getRegistro_trator(), placa_trator = veiculo.getPlaca_trator(),
						eixos_trator = veiculo.getEixos_trator(), tipo_trator = veiculo.getTipo_trator(),
						cidade_trator = veiculo.getCidade_trator(), uf_trator = veiculo.getUf_trator();
				String registro_reboque1 = veiculo.getRegistro_reboque1(), placa_reboque1 = veiculo.getPlaca_reboque1(),
						eixos_reboque1 = veiculo.getEixos_reboque1(), tipo_reboque1 = veiculo.getTipo_reboque1(),
						cidade_reboque1 = veiculo.getCidade_reboque1(), uf_reboque1 = veiculo.getUf_reboque1();
				String registro_reboque2 = veiculo.getRegistro_reboque2(), placa_reboque2 = veiculo.getPlaca_reboque2(),
						eixos_reboque2 = veiculo.getEixos_reboque2(), tipo_reboque2 = veiculo.getTipo_reboque2(),
						cidade_reboque2 = veiculo.getCidade_reboque2(), uf_reboque2 = veiculo.getUf_reboque2();
	...
	public boolean atualizar(int flag_armazem) {
		...
		if (permitir_cadastro) {
			...
			if (permitir_cadastro) {
				if (cliente_atualizar.getVeiculos().size() > 0 && cliente_atualizar != null) {
					...
					if (atualizar.inserirVeiculos(cliente_atualizar.getVeiculos(), cliente_atualizar.getId()) == 1) {
						...
						JOptionPane.showMessageDialog(isto,
								"Erro ao deletar incluir veiculos, corrupção no banco de dados!");
						permitir_cadastro = false;
			...
			if (permitir_cadastro) {
				...
				JOptionPane.showMessageDialog(null, "RNTRC: " + cliente_atualizar.getRntrc() + " Status RNTRC: "
						+ cliente_atualizar.getStatus_cadastro());
