...
public class TelaAditivos extends JFrame {

	private final JPanel painelPrincipal = new JPanel();
    private TelaAditivos isto;
    private JTable tabela_aditivos;
    private ArrayList<CadastroAditivo> lista_aditivos = new ArrayList<>();
	private AditivoTableModel modelo_aditivos = new AditivoTableModel();

	public TelaAditivos(Window janela_pai) {

		 isto = this;
		
		setResizable(true);
		setTitle("E-Contract - Aditivos");
		...
		JLabel lblNewLabel = new JLabel("Aditivos");
		...
		tabela_aditivos = new JTable(modelo_aditivos);
		tabela_aditivos.setOpaque(false);
		tabela_aditivos.setAutoResizeMode(JTable.AUTO_RESIZE_ALL_COLUMNS);

		tabela_aditivos.setRowHeight(30);
		
		JScrollPane scrollPane = new JScrollPane(tabela_aditivos);
		...
		btnExcluir.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				int indiceDaLinha = tabela_aditivos.getSelectedRow();

				int id_contrato_selecionado = Integer.parseInt(tabela_aditivos.getValueAt(indiceDaLinha, 3).toString());
	...
	public void pesquisar() {
		...
		lista_aditivos.clear();
		modelo_aditivos.onRemoveAll();
		
		lista_aditivos = gerenciar.getAditivos();
		for(CadastroAditivo cc : lista_aditivos) {
			modelo_aditivos.onAdd(cc);
		}
	}
	...
	public class AditivoTableModel extends AbstractTableModel {
		...
		private final ArrayList<CadastroAditivo> dados = new ArrayList<>();// usamos como dados uma lista genérica de
																			// nfs
		...
		public Object getValueAt(int rowIndex, int columnIndex) {
			// pega o dados corrente da linha
			 CadastroAditivo dado = dados.get(rowIndex);


			switch (columnIndex) {
			case id:
				return dado.getId_aditivo();
			case status  :{
		...
		public void setValueAt(Object aValue, int rowIndex, int columnIndex) {
			CadastroAditivo ib = dados.get(rowIndex);
		 ...
		 * @return
		 */
		public CadastroAditivo getValue(int rowIndex) {
		 ...
		 * @return
		 */
		public int indexOf(CadastroAditivo dado) {
		 ...
		 * @param empregado
		 */
		public void onAdd(CadastroAditivo dado) {
			dados.add(dado);
		 ...
		 * @param dadosIn
		 */
		public void onAddAll(ArrayList<CadastroAditivo> dadosIn) {
		 ...
		 * @param empregado
		 */
		public void onRemove(CadastroAditivo dado) {
		...
		public CadastroAditivo onGet(int row) {
			return dados.get(row);
		}
	}
	
	public CadastroAditivo getCadastroAditivoSelecionado() {
		int indiceDaLinha = tabela_aditivos.getSelectedRow();

	
		return modelo_aditivos.getValue(indiceDaLinha);
