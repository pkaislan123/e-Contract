...
public class TelaEnviarMsgWhatsapp extends JFrame {
	...
	private String mensagem_notificacao_global, mensagem_anexo_global, nome_comprador_global, mensagem_anexo_comprador,
			mensagem_notificacao_comprador, mensagem_anexo_vendedor1, mensagem_notificacao_vendedor1,
			mensagem_anexo_vendedor2, mensagem_notificacao_vendedor2, nome_vendedor1_global, nome_vendedor2_global;
	...
	public TelaEnviarMsgWhatsapp(CadastroContrato contrato, Window janela_pai) {
		...
		System.out.println("Nome do comprador: " + compradores[0].getNome_empresarial() + "outro nome: "
				+ compradores[0].getNome_fantaia());
	...
	public void set_msg_notificacao(int flag_cliente) {
		...
		if (flag_cliente == 1) {
			mensagem = "Ola! \\n \\nSou " + login.getNome() + " " + login.getSobrenome() + ","
					...
					+ nome_comprador_global + " , assim sendo, envio essa mensagem para notifica-lo que o contrato de "
			...
			mensagem = "Ola! \\n \\nSou " + login.getNome() + " " + login.getSobrenome() + ","
					+ "  represento a LD Armazens de Guarda-Mor/MG. \\n\\nEste  numero esta cadastrado em nossa base de dados para entrar em contato com o produtor rural "
					+ nome_vendedor1_global + " , assim sendo, envio essa mensagem para notifica-lo que o contrato de "
					+ contrato_local.getQuantidade() + " " + contrato_local.getMedida() + " de "
					+ contrato_local.getModelo_safra().getProduto().getNome_produto() + " firmado na data de "
					+ contrato_local.getData_contrato()
					+ " ainda encontra na nossa base de dados com a carencia de assinatura"
			...
			mensagem = "Ola! \\n \\nSou " + login.getNome() + " " + login.getSobrenome() + ","
					+ "  represento a LD Armazens de Guarda-Mor/MG. \\n\\nEste  numero esta cadastrado em nossa base de dados para entrar em contato com o produtor rural "
					+ nome_vendedor2_global + " , assim sendo, envio essa mensagem para notifica-lo que o contrato de "
					+ contrato_local.getQuantidade() + " " + contrato_local.getMedida() + " de "
					+ contrato_local.getModelo_safra().getProduto().getNome_produto() + " firmado na data de "
					+ contrato_local.getData_contrato()
					+ " ainda encontra na nossa base de dados com a carencia de assinatura"
	...
	public void enviar_Mensagem() {
		...
		String caminho_completo = unidade_base_dados + "\\" + contrato_local.getCaminho_diretorio_contrato()
				+ "\\" + contrato_local.getNome_arquivo();

		System.out.println("url do arquivo para a nuvem: " + caminho_completo);

		nuvem.deletarArquivo(contrato_local.getNome_arquivo());

		boolean carregar_arquivo = nuvem.carregar(caminho_completo,  contrato_local.getNome_arquivo());
		...
		if (chkOutroContato.isSelected()) {
			...
			String celular_neutro = etNumero.getText();
