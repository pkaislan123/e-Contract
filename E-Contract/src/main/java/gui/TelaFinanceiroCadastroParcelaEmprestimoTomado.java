...
public class TelaFinanceiroCadastroParcelaEmprestimoTomado extends JFrame {
	...
	private TelaFinanceiroCadastroParcelaEmprestimoTomado isto;
	...
	private JButton btnApagarPagamento;
	private  ParcelaEmprestimo parcela_global;
	...
	JRadioButton rdbtnRecebedorIB = new JRadioButton("Uma Instituição Bancaria?");
	JRadioButton rdbtnRecebedorCliente = new JRadioButton("Um Cliente/Forncedor?");
	JRadioButton rdbtnPagadorIB = new JRadioButton("Uma Instituição Bancaria?");
	JRadioButton rdbtnUmClienteforncedor = new JRadioButton("Um Cliente/Forncedor?");
	
	JRadioButton rdbtnPagamentoFluxoDeCaixaSim = new JRadioButton("SIM");
	JRadioButton rdbtnPagamentoExtratoSim = new JRadioButton("SIM");
	JRadioButton rdbtnPagamentoFluxoDeCaixaNao = new JRadioButton("NÃO");
	JRadioButton rdbtnPagamentoExtratoNao = new JRadioButton("NÃO");
	
	public TelaFinanceiroCadastroParcelaEmprestimoTomado(int modo_operacao, ParcelaEmprestimo parcela,
		...
		setResizable(true);
		if (modo_operacao == 0)
			setTitle("E-Contract - Cadastro Parcela de Empréstimo Tomando");
		else if (modo_operacao == 1)
			setTitle("E-Contract - Edição Parcela de Empréstimo Tomando");

		parcela_global = parcela;
		
		setBackground(new Color(255, 255, 255));
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 1018, 715);
			...
			public void actionPerformed(ActionEvent e) {
				...
				if ((parcela_atualizar != null && parcela_atualizar.getCriar_pagamento() == 0) ||
					...
					if (result) {
						...
						if(parcela.getCriar_pagamento() == 1  &&   parcela_atualizar.getCriar_pagamento() == 1) {
							//parcela ja existia um pagamento, agora atualizar
							...
							if(id_pagamento_inserido > 0) {
								...
								boolean atualizado = gerenciar.atualizarIdPagamento(parcela_atualizar.getId_parcela(), id_pagamento_inserido);
								if(atualizado) {
...
if(pag != null) {
					...
					if(pag.getTipo_pagador() == 0) {
						...
						InstituicaoBancaria ib = gerenciar_ibs.getInstituicaoBancaria(pag.getId_pagador());
						if (ib != null)
						...
						CadastroCliente cli = gerenciar_clientes.getCliente(pag.getId_pagador());
						if (cli != null)
							setPagadorClienteFornecedor(cli);
					}
					
					if(pag.getTipo_recebedor() == 0) {
						rdbtnRecebedorIB.setSelected(true);
						rdbtnRecebedorCliente.setSelected(false);

						InstituicaoBancaria ib = gerenciar_ibs.getInstituicaoBancaria(pag.getId_recebedor());
						if (ib != null)
						...
						CadastroCliente cli = gerenciar_clientes.getCliente(pag.getId_recebedor());
						if (cli != null)
				...
				if (rdbtnPagamentoSim.isSelected()) {
					...
					pagamento_da_parcela.setId_lancamento(id_lancamento_pai);
						...
						if(rdbtnPagadorIB.isSelected()) {
							...
							if (pagador_ib != null) {
								pagamento_da_parcela.setId_pagador(pagador_ib.getId_instituicao_bancaria());
							...
							pagamento_da_parcela.setTipo_pagador(1);
							if (pagador_cliente_fornecedor != null) {
								pagamento_da_parcela.setId_pagador(pagador_cliente_fornecedor.getId());
						...
						if(rdbtnRecebedorIB.isSelected()) {
							pagamento_da_parcela.setTipo_recebedor(0);
							if (recebedor_ib != null) {
								pagamento_da_parcela.setId_recebedor(recebedor_ib.getId_instituicao_bancaria());
							...
							if (recebedor_cliente_fornecedor != null) {
								pagamento_da_parcela.setId_recebedor(recebedor_cliente_fornecedor.getId());
...
*/
	...
	public void setPainelPagamento() {
		...
		//painelPagamento.add(panel, "cell 1 0,growx");
		painelPagamento.add(panel, "cell 1 1,growx");

		JLabel lblPagador = new JLabel("Pagador:");
		lblPagador.setFont(new Font("Tahoma", Font.PLAIN, 16));
		painelPagamento.add(lblPagador, "cell 0 2,alignx trailing");

		 cBPagador = new JComboBox();
		painelPagamento.add(cBPagador, "cell 1 2,growx");
		...
		btnSelecionarPagador.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (rdbtnPagadorIB.isSelected()) {
					TelaFinanceiroInstituicaoBancaria tela = new TelaFinanceiroInstituicaoBancaria(0, 6, isto);
					tela.setVisible(true);
			
				
				} else if (rdbtnUmClienteforncedor.isSelected()) {
					TelaCliente tela = new TelaCliente(0, 51, isto);
					tela.setVisible(true);
				}
		...
		painelPagamento.add(btnSelecionarPagador, "cell 2 2");
		...
		painelPagamento.add(panel_2, "cell 1 3,growx");
		
		
		
		JLabel lblRecebedor = new JLabel("Recebedor:");
		lblRecebedor.setFont(new Font("Tahoma", Font.PLAIN, 16));
		painelPagamento.add(lblRecebedor, "cell 0 4,alignx trailing");

		 cBRecebedor = new JComboBox();
		painelPagamento.add(cBRecebedor, "cell 1 4,growx");
		...
		btnSelecionarRecebedor.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (rdbtnRecebedorIB.isSelected()) {
					TelaFinanceiroInstituicaoBancaria tela = new TelaFinanceiroInstituicaoBancaria(0, 7, isto);
					tela.setVisible(true);
			
				
				} else if (rdbtnRecebedorCliente.isSelected()) {
					TelaCliente tela = new TelaCliente(0, 52, isto);
					tela.setVisible(true);
				}
		...
		painelPagamento.add(btnSelecionarRecebedor, "cell 2 4");

		JLabel lblCondioDoPagamento = new JLabel("Condição do Pagamento:");
		lblCondioDoPagamento.setFont(new Font("Tahoma", Font.PLAIN, 16));
		painelPagamento.add(lblCondioDoPagamento, "cell 0 5,alignx trailing");

		
		 cBCondicaoPagamento = new JComboBox();
		painelPagamento.add(cBCondicaoPagamento, "cell 1 5,growx");
		...
		btnSelecionarCondicaoPagamento.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				TelaFinanceiroCondicaoPagamento tela = new TelaFinanceiroCondicaoPagamento(0, 6, isto);
				tela.setVisible(true);
			}
		});
		...
		painelPagamento.add(btnSelecionarCondicaoPagamento, "cell 2 5");

		JLabel lblStatusDoPagamento = new JLabel("Status do Pagamento:");
		lblStatusDoPagamento.setFont(new Font("Tahoma", Font.PLAIN, 16));
		painelPagamento.add(lblStatusDoPagamento, "cell 0 6,alignx trailing");
		...
		painelPagamento.add(cBStatusPagamento, "cell 1 6,growx");

		btnApagarPagamento = new JButton("Remover Pagamento");
		btnApagarPagamento.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
            //excluir pagamento
				excluirPagamento();
			}
		});
		btnApagarPagamento.setForeground(Color.WHITE);
		btnApagarPagamento.setFont(new Font("SansSerif", Font.BOLD, 14));
		btnApagarPagamento.setBackground(new Color(0, 0, 204));

		painelPagamento.add(btnApagarPagamento, "cell 1 7,growx");
	...
	public void excluirPagamento() {
		if (JOptionPane.showConfirmDialog(isto, "Deseja excluir o Pagamento desta parcela?", "Excluir", JOptionPane.YES_NO_OPTION,
				JOptionPane.QUESTION_MESSAGE) == JOptionPane.YES_OPTION) {


					boolean exclusao = new GerenciarBancoFinanceiroPagamento().removerFinanceiroPagamento(parcela_global.getId_pagamento());
					if (exclusao) {
					    
						//atualizar esta parcela como criarPagamento = 0 e id_pagamento = 0;
						boolean remover_pagamento =  new GerenciarBancoParcelasEmprestimo().removerPagamentoDaParcela(parcela_global.getId_parcela());
						if(remover_pagamento) {
							JOptionPane.showMessageDialog(isto, "Pagamento Excluído e Parcela Atualizada");
					...
					((TelaFinanceiroGerenciarLancamento) janela_pai_global).atualizar_informacoes();
