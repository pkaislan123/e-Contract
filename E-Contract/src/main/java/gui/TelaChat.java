...
public class TelaChat extends JDialog {
	...
	private int ultimo_valor = 0;
	private boolean thread_criada = false;
	...
	public TelaChat(Window janela_pai) {
		...
		painelMensagens.setBounds(10, 51, 417, 573);
		...
		abaConversas.setBounds(0, 0, 417, 573);
	...
	public boolean temAba(CadastroLogin login_selecionado) {
	...
	public boolean temRegistro(CadastroLogin login_selecionado) {
			...
			public void run() {
				...
				while (true) {
					...
					for (CadastroLogin login_selecionado : usuarios_encontrados) {
						if (login_selecionado.getId() != login.getId() && login_selecionado.getStatus() == 1) {

							if (!temAba(login_selecionado)) {
								...
								ArrayList<CadastroLogin.Mensagem> mensagens = gerenciarUsuarios
										.getMensagens(login.getId(), login_selecionado.getId());

								if (!temRegistro(login_selecionado)) {
								...
								listModel.addAll(mensagens); // adiciona as mensagens na lista
									...
									public void actionPerformed(ActionEvent e) {
										...
										if (enviado) {
											...
											scrollPane.getViewport()
													.setViewPosition(new Point(0, maxValue));
										}
										enviar_mensagem.setText("");
										enviar_mensagem.setFocusable(true);
								...
								new Thread() {
									...
									public void run() {
										...
										while (true) {
											...
											// pega as mensagens
											// JOptionPane.showMessageDialog(null, "procurando mensagens da conversa
											// entre "
											// + login.getLogin() + " e " + login_selecionado.getLogin());
											if (gerenciarMensagens == null)
												gerenciarMensagens = new GerenciarBancoLogin();
											//mensagens = gerenciarMensagens.getMensagens(login.getId(),
												//	login_selecionado.getId());
											int num_atual_msgs = gerenciarMensagens.getNumMensagens(login.getId(),login_selecionado.getId() );	


											RegistroMensagem registro = new RegistroMensagem();
											registro.setLogin_selecionado(login_selecionado);
											/*
											 * JOptionPane.showMessageDialog(null, "numero de mensagens encontradas:  "
											 * + mensagens.size());
											 */
											//registro.setNum_mensagens_atual(mensagens.size());
											registro.setNum_mensagens_atual(num_atual_msgs);
											...
											if (!ja_registrado) {
												...
												// mensagem nova verificando o numero de mensagens antigo com o lido
												// agora
												/*
												 * JOptionPane.showMessageDialog(null,
												 * "Este login ja tem registro de mensagens, login : " +
												...
												if (registro_mensagens.get(indice).getNum_mensagens_atual() < registro
													...
													CadastroLogin.Mensagem ultima_mensagem = listModel
															.get(listModel.size() - 1);

													if (!(ultima_mensagem.getId_remetente() == login.getId())) {
														CadastroLogin login_msg = gerenciarMensagens
																.getLogin(ultima_mensagem.getId_remetente());
							...
							} // fecha a verificacao se tem aba
