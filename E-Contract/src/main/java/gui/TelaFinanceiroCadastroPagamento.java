...
public class TelaFinanceiroCadastroPagamento extends JFrame {
	...
	public TelaFinanceiroCadastroPagamento(int modo_operacao, FinanceiroPagamento pagamento, Lancamento lancamento_pai,
			Window janela_pai) {
		...
		rdbtnRecebedorCliente = new JRadioButton("Um Cliente/Forncedor?");
		...
		if (modo_operacao == 0) {
			...
			if(lancamento_pai.getTipo_lancamento() == 0) {
				//é um lancamento do tipo despesa, o recebedor e o prorio recebedor do lancamento
				if(lancamento_pai.getId_cliente_fornecedor() > 0) {
					CadastroCliente recebedor = new GerenciarBancoClientes().getCliente(lancamento_pai.getId_cliente_fornecedor());
					if(recebedor != null) {
						rdbtnRecebedorCliente.setSelected(true);
						rdbtnRecebedorIB.setSelected(false);
						
						setRecebedorClienteFornecedor(recebedor);
