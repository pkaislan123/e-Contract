...
public class TelaEnviarMsgMail extends JFrame {
	...
	private String nome_comprador_global, mensagem_anexo, mensagem_notificacao, mensagem_assinatura, mensagem_assunto,
			mensagem_saudacao;
	...
	public TelaEnviarMsgMail(int flag_tipo_envio, CadastroContrato contrato, File documento, Window janela_pai) {
		...
		setResizable(true);
		setTitle("E-Contract - Enviar Documento Via E-Mail");

		setBackground(new Color(255, 255, 255));
		setDefaultCloseOperation(JFrame.DISPOSE_ON_CLOSE);
		setBounds(100, 100, 1137, 650);
		...
		chkAnexarContrato = new JCheckBox("Anexar Documento na Mensagem?");
		...
		btnNewButton_2.setBounds(1006, 99, 89, 23);
		...
		mensagem_anexo = "\n\nSou " + login.getNome() + " " + login.getSobrenome() + ","
				...
				+ nome_comprador_global.toUpperCase()
				+ " , assim sendo, envio essa mensagem para notifica-lo que o contrato de "
				+ contrato_local.getQuantidade() + " " + contrato_local.getMedida() + " de "
				+ contrato_local.getModelo_safra().getProduto().getNome_produto() + " firmado na data de "
				+ contrato_local.getData_contrato()
				+ " ainda encontra na nossa base de dados com a carencia de assinatura"
		...
		mensagem_envio_relatorio = "Em anexo a esta mensagem envio o relatorio referente ao contrato "
				+ contrato_local.getCodigo() + " entre o comprador " + nome_comprador_global + " e o(s) vendedor(es) "
				+ nome_vendedores_global + " na quantidade de " + contrato_local.getQuantidade() + " "
				+ contrato_local.getMedida() + " de " + contrato_local.getModelo_safra().getProduto().getNome_produto()
				+ " firmado na data de " + contrato_local.getData_contrato();
	...
	public void enviar_email() {
		...
		if (rdNot.isSelected()) {
			...
			if (checkEmail(destinatario)) {
				// public boolean enviar(String remetente, String destinatario, String assunto,
				// String saudacao, String mensagem, String assinatura) {
			...
			// enviarAnexo(String remetente, String destinatario, String assunto, String
			// saudacao, String mensagem, String assinatura, ArrayList<File> anexos) {
			...
			if (checkEmail(destinatario)) {
				...
				boolean enviado = mail.enviarAnexo(remetente, destinatario, assunto, saudacao, mensagem, assinatura,
						arquivos);
				if (enviado == true) {
					JOptionPane.showMessageDialog(isto, "E-mail enviado");
					isto.dispose();
