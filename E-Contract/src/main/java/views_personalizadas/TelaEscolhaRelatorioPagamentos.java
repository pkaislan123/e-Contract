...
public class TelaEscolhaRelatorioPagamentos extends JDialog {

	private TelaEscolhaRelatorioPagamentos isto;
	 ...
	 */
	public TelaEscolhaRelatorioPagamentos(ArrayList<FinanceiroPagamentoCompleto> pagamentos, Window janela_pai) {
		...
		getContentPane().add(panel_2, "cell 0 0 2 1,grow");
				...
				JLabel btnRelatrioDelancamentos = new JLabel("Relatório de Pagamentos");
							...
							public void actionPerformed(ActionEvent e) {
								...
								if (rdbtnCompleto.isSelected()) {
									...
									if (rdbtnExcel.isSelected()) {
										gerarExcel(prepararCompleto(pagamentos, 1));
									} else if (rdbtnPdf.isSelected()) {
										gerarPdf(prepararCompleto(pagamentos, 1));
									...
									// relatorio simples
									if (rdbtnExcel.isSelected()) {
										gerarExcel(preparar(pagamentos, 0));
									} else if (rdbtnPdf.isSelected()) {
										gerarPdf(preparar(pagamentos, 0));
	...
	public TelaEscolhaRelatorioPagamentos(ArrayList<FinanceiroPagamentoCompleto> pagamentos, TelaFinanceiroPagamentoInternal janela_pai) {
		getContentPane().setBackground(Color.WHITE);
	...
	public HSSFWorkbook preparar(ArrayList<FinanceiroPagamentoCompleto> pagamentos_selecionados, int flag) {
		HSSFWorkbook workbook = new HSSFWorkbook();
		HSSFSheet sheet = workbook.createSheet("Exportação de Dados de Pagamentos");
		...
		// Configurando as informacoes
				...
				cell.setCellValue("Relatório de Pagamentos");
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("IDEN. GERAL".toUpperCase());

		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("PAGADOR".toUpperCase());

		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("RECEBEDOR".toUpperCase());

		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("DATA PAG.".toUpperCase());
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("COND. DE PAG.".toUpperCase());

		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("STS COND. DE PAG.".toUpperCase());

		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("EXTRATO".toUpperCase());
		...
		for (FinanceiroPagamentoCompleto dado : pagamentos_selecionados) {
		
			if (dado.getLancamento().getTipo_lancamento() == 0) {
				// despesas a pagar
				valor_total_despesas = valor_total_despesas.add(dado.getFpag().getValor());
			} else if (dado.getLancamento().getTipo_lancamento() == 1) {
				// receita
				valor_total_receitas = valor_total_receitas.add(dado.getFpag().getValor());

			} 
			 else if (dado.getLancamento().getTipo_lancamento() == 2) {
					// receita
				 valor_total_transferencias = valor_total_transferencias.add(dado.getFpag().getValor());

			} 
			
			else if (dado.getLancamento().getTipo_lancamento() == 3) {
				// emprestimo
				if(dado.getFpag().getTipo_pagamento() == 1) {
					valor_total_pagamentos_emprestimo_despesas = valor_total_pagamentos_emprestimo_despesas.add(dado.getFpag().getValor());

				}else {
					valor_total_pagamentos_emprestimo_receitas = valor_total_pagamentos_emprestimo_receitas.add(dado.getFpag().getValor());
				}
			}
			row = sheet.createRow(rownum++);
			cellnum = 0;
			...
			cell.setCellValue(dado.getFpag().getId_pagamento());
			...
			cell = row.createCell(cellnum++);
			cell.setCellStyle(textStyle);
			cell.setCellValue(dado.getFpag().getIdentificador());
		
		
		//pagador
			String pagador  = "";
			try {
				pagador = dado.getNome_pagador().toUpperCase();
			...
			String recebedor  = "";
			try {
				recebedor = dado.getNome_recebedor().toUpperCase();
		...
		//data pagamento
			...
			if(dado.getFpag().getData_pagamento() != null && !dado.getFpag().getData_pagamento().equalsIgnoreCase("") ) {
		         data = dado.getFpag().getData_pagamento();
			}
			...
			cell = row.createCell(cellnum++);
			cell.setCellStyle(textStyle);
			cell.setCellValue(NumberFormat.getCurrencyInstance(ptBr).format(dado.getFpag().getValor()));
			...
			cell = row.createCell(cellnum++);
			cell.setCellStyle(textStyle);
			cell.setCellValue(s_condicao);
			
			String sts_condicao_pag = "";

			try {
				int id_status = dado.getFpag().getStatus_pagamento();
				if (id_status == 0) {
					sts_condicao_pag += ("A - Compensar|Realizar|Concluir;");
				} else if (id_status == 1) {
					sts_condicao_pag += ("Compensado|Realizado|Concluído;");
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(textStyle);
		cell.setCellValue(sts_condicao_pag);
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue("Núm. Total de Pagamentos:");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));

		cell = row.createCell(3);
		cell.setCellStyle(celula_fundo_laranja_texto_branco);
		cell.setCellValue(pagamentos_selecionados.size());
		...
		cell.setCellValue("Valor Total Pagamento Despesas:");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(negrito_direita);
		cell.setCellValue("Valor Total Pagamento Receitas:");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(negrito_direita);
		cell.setCellValue("Valor Total Pagamento Transferência:");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(negrito_direita);
		cell.setCellValue("Valor Total Pagamento Emprestimos(Despesas):");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(negrito_direita);
		cell.setCellValue("Valor Total Pagamento Emprestimos(Receitas):");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));

		cell = row.createCell(3);
		cell.setCellStyle(textStyleVerde);
		cell.setCellValue(NumberFormat.getCurrencyInstance(ptBr).format(valor_total_pagamentos_emprestimo_receitas));
		...
		cell = row.createCell(cellnum++);
		cell.setCellStyle(negrito_direita);
		cell.setCellValue("Balanço(Emprestimos):");
		 sheet.addMergedRegion(new CellRangeAddress(rownum, rownum, 0, 2));

		cell = row.createCell(3);
		cell.setCellStyle(textStyle);
		cell.setCellValue(NumberFormat.getCurrencyInstance(ptBr).format(balanco_emprestimo));
	...
	public HSSFWorkbook prepararCompleto(ArrayList<FinanceiroPagamentoCompleto> pagamentos_selecionados, int flag) {
		HSSFWorkbook workbook = new HSSFWorkbook();
